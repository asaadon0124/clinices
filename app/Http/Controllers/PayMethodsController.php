...
class PayMethodsController extends Controller
{
        ...
        if ($reservation->payment_method == 'paypal') {
            ...
            $response = $provider->createOrder(
                [
                    ...
                    [
                        // frontend_Url
                        "return_url" => url('http://localhost:3000/my-appointments'),
                        "cancel_url" => url('http://localhost:3000/'),
                    ],
                    "purchase_units" =>
                    [
                        [
    ...
    public function cancel_paypal_payment()
    {
        ...
        $session = Session::create([
            ...
            'line_items' => [[
                'price_data' => [
                    ...
                    'unit_amount' => $reservation->feese->price * 100, // السعر بالسنتات
                ],
                'quantity' => 1,
            ]],
            'mode' => 'payment',
            'success_url' => 'http://localhost:3000/my-appointments?session_id={CHECKOUT_SESSION_ID}',
